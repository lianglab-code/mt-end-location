function mouse_over_image_circle(img)
%% MOUSE_OVER_IMAGE moves a circle on an image figure as the mouse
% moves.
...
% generate a circular patch
c = [w/2;h/2];
circ = helper_gen_circ(c,patch_radius);
h_patch = patch(circ(1,:),circ(2,:),'g','parent',h_ax2,'facealpha', ...
...
for ii = 1:numel(h_child)
    if isgraphics(h_child(ii),'image')
        ...
        bw = poly2mask(c(1,:),c(2,:), ...
                       size(alpha_data,2), ...
                       size(alpha_data,1));
        alpha_data = helper_mask(alpha_data,bw,0);
...
for ii = 1:numel(h_child)
    if isgraphics(h_child(ii),'image')
        ...
        bw = poly2mask(c(1,:),c(2,:), ...
                       size(alpha_data,2), ...
                       size(alpha_data,1));
        alpha_data = helper_mask(alpha_data,bw,p_a);
        set(h_child(ii),'alphadata',alpha_data);
    end
end
