function play_gui_with_point2(img, cell_centers)
%% PLAY_GUI play image as movies
% img: the images to be played
% cell_centers: cell array containg complex point arraies

% INPUT:
% cell_centers: 1*num_frame cell, each cell is the vector of
%               complex number for centers
...
%% UI
    ...
    if(~iscell(cell_centers))
        msgbox(['cell_centers must be a cell array']);
        return;
    end

    np = numel(cell_centers);
    if(num_frames~=np)
        msgbox(['the size of cell array cell_centers must ' ...
                'be equal to image frame number']);
        return;
    end
    
    nl = 0;
    for i = 1:np
        if(nl<numel(cell_centers{i}))
            nl = numel(cell_centers{i});
        end
    end
    
    centers=complex(zeros(np,nl));
    for i = 1:np
        for j=1:numel(cell_centers{i})
            centers(i,j) = cell_centers{i}(j);
        end
    end
